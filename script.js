...
function scoreInfo() {
  ...
  const dataReceived = localStorage.getItem("dataBestScore");
  return JSON.parse(dataReceived);
};

const dataPlayer = scoreInfo();

const dataResultValue = {

  best: dataPlayer ? dataPlayer.best : 0
};

bestScore.innerHTML = `Best Score : ${dataResultValue.best}`;
best_score = Number(dataResultValue.best);

const storeDataValuesToLocalStorage = function() {

  const dataToSave = JSON.stringify(dataResultValue);
  localStorage.setItem("dataBestScore", dataToSave);
};
...
function setNumberSize(num, cel){
  if(num < 9 ) {
    ...
    cel.style.fontSize = `${Math.round((s[0].clientWidth * 43) / 100)}px`;
...
function setNumberColor(num, cel){
  ...
  if(num === 2) {
    cel.style.backgroundColor = "rgb(243, 243, 243)";
    cel.style.color = "grey";

  }
  if(num === 4) {
    cel.style.backgroundColor = "rgb(233,233,233)";
    cel.style.color = "grey";
  }
  if(num === 8) {
  ...
  if(num === 128) {
    cel.style.backgroundColor = "rgb(230, 186, 254)"; 
    cel.style.color = "white";
  }
  if(num === 256) {
    cel.style.backgroundColor = "rgb(206, 129, 247)";
    cel.style.color = "white";
  }
  if(num === 512) {
    cel.style.backgroundColor = "rgb(180, 51, 255)";
    cel.style.color = "white";
  }
  if(num === 1024) {
    ...
    if(w && interruttoreGenerale){
     ...
     if(gameOverEngine()) {
        ...
        if(Number(score) > Number(best_score)) {
          ...
          dataResultValue.best = Number(score);
...
touchArea.addEventListener("touchstart", (event) => {
  ...
  if(event.touches.length === 2) {
    ...
    isSwiped = true;
    initialX = event.touches[0].pageX - rectLeft; 
    initialY = event.touches[0].pageY - rectTop;
  }
...
touchArea.addEventListener("touchmove", (event) => {
    // e.prevent rimuove gli effetti di buonce della pagina
    if(event.touches.length === 1) {
      event.preventDefault();
      finalX = event.touches[0].pageX - rectLeft;
      finalY = event.touches[0].pageY - rectTop;
    }
